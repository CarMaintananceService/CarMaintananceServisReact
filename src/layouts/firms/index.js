  ...
  const mainDataSource = new CustomStore({
    ...
    load: (loadOptions) => {
      try {
        return axiosInstance
          .post("/Firm/Filter", prepareFilterBody(loadOptions))
          .then((response) => {
        ...
        console.error("Firm/Filter:", error);
      }
    },
    insert: (values) => {
      return axiosInstance
        .post("/Firm/InsertOrUpdate", values)
        .then((res) => {
          res = res.data;
        ...
        .catch((error) => {
          console.error("Firm/InsertOrUpdate : " + error);
          throw "Hatalı işlem!";
        });
    },
    update: (key, values) => {
      ...
      return axiosInstance
        .post("/Firm/InsertOrUpdate", post_values)
        .then((res) => {
          res = res.data;
    ...
    remove: (key) => {
      return axiosInstance
        ...
        .catch((error) => {
          console.error(`Firm/Delete/${key}/delete : `, error);
          throw "Hatalı işlem!";
        });
    },
  });
      ...
      <DashboardLayout>
        ...
        >
          ...
          <Column dataField="Telephone" caption="Telephone">
            <Editing
              validationRules={[
                { type: "required", message: "Telefon numarası zorunludur." },
                { type: "stringLength", max: 15, message: "Maksimum 15 karakter" },
              ]}
            />
          </Column>
          <Column dataField="TaxNumber" caption="TaxNumber">
            <Editing
              validationRules={[
                { type: "required", message: "?" },
                { type: "stringLength", max: 250, message: "Maksimum 250 karakter" },
